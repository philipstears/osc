...
fn main() -> Result<()> {
    ...
    let mut bpb_bytes = vec![0u8; BIOS_PARAMETER_BLOCK_SIZE];
    f.read_exact(bpb_bytes.as_mut_slice())?;

    println!(
        "OEM: {}",
        str::from_utf8(CommonBiosParameterBlock::read_oem(bpb_bytes.as_slice())).unwrap()
    );
